...
public class TechGenerator extends SimpleItemContainerMachine implements Radioactive {
  ...
  public static final ItemStack[] RECIPE_TECH_GENERATOR = {SupremeComponents.INDUCTIVE_MACHINE.asQuantity(1),
      SupremeComponents.SYNTHETIC_RUBY.asQuantity(1), SupremeComponents.INDUCTIVE_MACHINE.asQuantity(1),
      SlimefunItems.REINFORCED_ALLOY_INGOT.asQuantity(1),
      new ItemStack(Material.LOOM), SlimefunItems.REINFORCED_ALLOY_INGOT.asQuantity(1),
      SupremeComponents.CARRIAGE_MACHINE.asQuantity(1),
      SlimefunItems.HEATING_COIL.asQuantity(1), SupremeComponents.CARRIAGE_MACHINE.asQuantity(1)};
  ...
  private static ItemStack getCardTier(int tierCard) {
    if (tierCard >= 3) {
      return SupremeComponents.CENTER_CARD_ULTIMATE.asQuantity(1);
    } else if (tierCard == 2) {
      return SupremeComponents.CENTER_CARD_ADVANCED.asQuantity(1);
    } else {
      return SupremeComponents.CENTER_CARD_SIMPLE.asQuantity(1);
    }
  }
